...
## zc-walker-multi-branch-fanout-no-dispatch

generator capability gap: the walker supports only one non-message case target that itself fans into a further dispatch; this model's category dispatch fans into three

first error: Model 'Finra.Trace.Atds.Dfi.v2.1': no ZeroCopy walker for tree 'packet': no path from its root to a message dispatch. `ZeroCopy/Scaled.CSharp.ZeroCopy/CSharp/Framing/Walk.cs:14-34,111-123` (`Continue`) finds the outermost Branch-bearing element `packet.message.payload`, whose three case targets (`trademessage`, `administrativemessage`, `controlmessage`) are Groups, not messages (`HasConcreteMessageTargets.For` is false for all three). Each of the three itself contains exactly one further Branch-bearing element (`trademessagepayload`, `administrativemessagepayload`, `controlmessagepayload` respectively — confirmed by walking `Models/Binary/Finra/Finra.Trace.Atds.Dfi.v2.1.binary.model.json`), so `containers.Count` is 3. `Continue` only extends the walk when `containers.Count == 1` (`Walk.cs:122`), by design — the class comment states a case target that is not itself a message "must contain exactly one further Branch-bearing element", singular, implying at most one non-message case target per dispatch. This model's category dispatch (Trade / Administrative / Control) legitimately fans out into three independent message-type dispatches, one per category — a real three-way wire shape, not a malformed model. New declaration (first appearance in the corpus this run; `compile-binary-all` reports it `added`). Distinct from the other trees the generic `zc-no-walker-no-dispatch-path` key covers (header-only, GapDetection, transport-prefix trees with no reachable dispatch at all) — this tree has a dispatch, several of them, just more than the walker's `Continue` recursion currently follows. Fix site: `ZeroCopy/Scaled.CSharp.ZeroCopy/CSharp/Framing/Walk.cs`'s `Continue`/`OutermostBranchBearing` would need to follow every non-message case target that itself contains exactly one further Branch-bearing element, not just a lone one. Covers Finra.Trace.Atds.Dfi.v2.1.
...
## classes-businessheader-field-not-enumerated

generator defect: `AllStaticFields.For` only walks the frame-level header, each message, and homogeneous groups; a static prefix Group sitting between the message dispatch's container and the dispatch itself is visited by neither

first error: Model 'Tmx.Tsx.QuantumFeedLevel2.Xmt.v2.1': Field 'packet.body.bodymessage.businessheader.msgversion' has no generated shared field class for declaration 'MsgVersion' and wire digest 'Integer:1B:U:BE'. Model defect. Thrown from `Classes/Scaled.CSharp.Classes/CSharp/Field/FieldClassNameMap.cs:50` (`For`), which looks up a field's shared class in a map built once by `FieldClassNames.BuildMap` from `AllStaticFields.For(context)` (`Classes/Scaled.CSharp.Classes/CSharp/Field/AllStaticFields.cs:15-28`). That enumeration walks three sources only: the frame-level header (`Framing.PacketHeader.For(context)`), each element of `context.Messages`, and `CSharp.Group.HomogeneousIn.For(context.Model)`. In this model's tree, `packet.body.bodymessage` is a Group holding two children: `businessheader` (a static, non-repeated prefix Group with fields `MsgVersion`, `SourceId`, `StreamId`, `Sequence0`, `Sequence1`) followed by `businessmessage`, a Branch dispatch whose case targets are the real messages. `businessheader` is not `Framing.PacketHeader.For`'s frame-level header (that is `packet.frameheader`, already walked and passing), is not itself one of `context.Messages` (the case targets under `businessmessage` are), and is not a homogeneous repeated group. Its fields are therefore never added to the map, yet whatever later renders `packet.body.bodymessage`'s struct calls `FieldClassNameMap.For` on `businessheader.msgversion` expecting an entry. This shape — a static per-body header field group sitting as a sibling of the dispatch, inside the dispatch's own container element, rather than inside the frame-level header or inside a specific message — is why only the two Level2 Xmt models hit this and the sibling Level1/TsxAlpha Xmt models (also carrying the `tmx-startofframe-missing-signedness` header gap, now fixed upstream) do not: those protocol versions do not nest a second static header between the message container and the dispatch. Not traced further into whether `businessmessage`'s case targets (i.e. `context.Messages`) are `packet.body.bodymessage.businessmessage.*` or `packet.body.bodymessage` itself — either way `businessheader` is outside every message's own subtree. Fix site: `Classes/Scaled.CSharp.Classes/CSharp/Field/AllStaticFields.cs`, `For` needs a fourth source that walks a static sibling Group of a dispatch container that is not itself part of the frame-level header or a message. Masked before this run by `tmx-startofframe-missing-signedness` (fixed upstream, presumably by adding the missing `Signedness` trait this key proposed) failing generation earlier on `packet.frameheader.startofframe`; fixing that unmasked this field-enumeration gap. Covers Tmx.Tsx.QuantumFeedLevel2.Xmt.v2.1, Tmx.Tsx.QuantumFeedLevel2.Xmt.v3.6.
...
## cboe-boe-loader-consolidation-regression

upstream defect (propose only): a same-day UniversalBinarySpecification refactor merged two Boe loaders under one identifier without updating every OmiSpecifications declaration or preserving the original loader's input format

first error: two shapes, both new regressions from the previous good compile. Six models (cboe.byxequities/bzxequities x2/edgaequities/edgxequities x2.binaryorderentry.boe) fail with `No Binary Specification Loader registered as 'Boe.Streamlined.Pdf'`; their `Reference.xml` declares `<Loader>Boe.Streamlined.Pdf</Loader>` (for example `OmiSpecifications/Cboe/Usa/BzxEquities/Order/Cboe.BzxEquities.BinaryOrderEntry.Boe.v2.3.Reference.xml:39`, set by commit `79a3f662` in June and unchanged since). The other six (cboe.bzxoptions/c1options/c2options/cboeequities/cfefutures/edgxoptions.binaryorderentry.boe) declare `<Loader>Boe.Pdf</Loader>` and fail with a wrapped `Exception has been thrown by the target of an invocation.` (no inner detail in the publish log). `UniversalBinarySpecification` commit `1e29d32e` ("Refactor orginal Boe loader", 2026-09-23, same day as the pull) renamed the `Boe.Streamlined.Pdf` loader (`Assemblies/Omi.Binary.Specification.Loaders/Boe/Boe.Streamlined.Pdf/...`) to `Boe.Pdf` (`Assemblies/Omi.Binary.Specification.Loaders/Boe/Boe.Pdf/Source/Source.cs:12`, `public const string Identifier = "Boe.Pdf";`) and, in the same commit, deleted the original `Boe.Pdf` loader's 30,416-line test fixture (`Cboe.Equities.Bzx.OrderEntry.Boe.v2.3.Pdf.xml`) while replacing its implementation with the renamed streamlined loader's code. The six `Boe.Streamlined.Pdf` declarations now point at a loader identifier that no longer exists; the six `Boe.Pdf` declarations now point at a loader identifier whose implementation changed underneath them and can no longer parse whatever shape their PDF-derived xml carries, throwing during construction. Propose either updating the six `<Loader>Boe.Streamlined.Pdf</Loader>` declarations in OmiSpecifications to `<Loader>Boe.Pdf</Loader>` to match the rename, or reverting/splitting the UniversalBinarySpecification consolidation so the original `Boe.Pdf` loader keeps its own identifier and implementation; not applied. Blocked on: the six exception rows need the inner exception text (the publish log only records the wrapped `TargetInvocationException` message) from a UniversalBinarySpecification/ScaledCompilers owner to confirm whether they are the same rename issue or a second defect introduced by the same commit. Covers cboe.byxequities.binaryorderentry.boe.v2.4.55, cboe.bzxequities.binaryorderentry.boe.v2.3, cboe.bzxequities.binaryorderentry.boe.v2.4.55, cboe.bzxoptions.binaryorderentry.boe.v2.10, cboe.c1options.binaryorderentry.boe.v2.10, cboe.c2options.binaryorderentry.boe.v2.10, cboe.cboeequities.binaryorderentry.boe.v2.3.7, cboe.cfefutures.binaryorderentry.boe.v1.3, cboe.edgaequities.binaryorderentry.boe.v2.4.55, cboe.edgxequities.binaryorderentry.boe.v2.3, cboe.edgxequities.binaryorderentry.boe.v2.4.55, cboe.edgxoptions.binaryorderentry.boe.v2.10.
